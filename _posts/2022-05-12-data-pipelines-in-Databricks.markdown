...
Delta lake enables developers to create Lakehouse architectures on top of data lakes.

Delta lake is open source.
Delta lakehouse combines elements from a data warehouse and a data lake.
...
- Dags are autogenerated

  This was a challenge because we have no flexibility to move dags so the pipeline can look messy.

- Unable to run streaming jobs in multi task jobs

- Unable to define dependencies between jobs

  When you have multiple pipelines you can’t specify dependencies between them.For example my stage two pipeline cannot run once my stage one pipeline completes because you can't define the dependency between the two pipelines.
...
<br/>
One of the testing tools used with Databricks is great expectations for data quality testing and this can be incorporated directly into the data pipeline.
...
You can set up your email to send notifications to slack.
