...
Architecting multi-tenant cloud native software can be quite challenging due to the broad scope of technical know-how required to execute such an undertaking. This article is an attempt to produce a distilled "how-to" guide for those who may find themselves bogged down while building multi-tenant applications. You can clone the repo containing the source code from this [github link](https://github.com/MechanicalRock/Multitenancy-AuthorizationAuthentication).

This article assumes you have a a basic understanding of lambda authorisers and JWTs. If not, refer to the source code's read me by clicking [here](https://github.com/MechanicalRock/Multitenancy-AuthorizationAuthentication)

This article assumes knowledge with by introducing lambda authorisers and the use of JWTs for user authorisation. If you are already familiar with lambda authorisers and JWTs you can skip ahead to a multi-tenancy example implementation of by clicking [here](#Multi-Tenancy-Example).
