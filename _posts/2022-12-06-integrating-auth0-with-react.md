...
Authentication and Authorization is objectively the most important part of any application, So why is it so often left to the end of development cycles or implemented in a slap dash manner?
...
Auth0 is one of the most highly reputed company working in this space. Why build your own service when you can get the years of experience and knowledge dedicated to making sure your users login data is safe?

Auth0 provides Authentication, Authorization, Social Logins, MFA, Analytics and lots more, And to make it all the sweeter, has amazing developer documentation. All of this completely free if you have under 7000 users!

## Create Auth0 Application

Let's start with create a new Auth0 Application
...
One last step before we can begin integrating! We need to let Auth0 know where we are allowed to send users after authentication or logout, These are the **Allowed Callback URLs** and the **Allowed Logout URLs**, in addition, if you would like Auth0 to handle refreshing the users token and session persistence, we will need to configure the **Allowed Web Origins** aswell.
...
> For this blog I will be working with **create-react-app** using the typescript template. The Auth0 SDK has full Typescript support in-built.
...
The SDK has everything you will need to ingtegrate your application with Auth0, Under the hood it uses React Context to keep track of your users state.
...
The `Auth0Provider` takes in two mandatory values, `domain` and `clientId`, Both of these can be found in your Settings of your Auth0 application. In addition, an optional but important parameter is the `redirectUri`, This will tell Auth0 where to send your users after authenticating successfully.
...
The button itself makes use of one of the amazing Hooks provided by the Auth0 SDK `useAuth0`
...
Firstly lets create a new Logout button.
...
Here we will once again utilize the `useAuth0` hook this time with the `logout` method, Pretty straight forward right?
...
It can also be configured with an optional parameter `returnTo` to direct them to another part of your application if your would like.
```ts
...
Now that our users can login, How do we go about getting thier information such as Name, Email address, Profile Picture etc?
...
Once again, the `useAuth0` hook showing it's strengths! We can get the user object, check if they are authenticated, and that everything has been fully loaded before displaying the user profile.
...
We have just touched the surface of what Auth0 can do.

Auth0 has great Analytics, User Management, Authorization, RBAC, MFA, Mutli-tennancy and many more

You can learn more at [Auth0](https://auth0.com/) or read thier [Docs](https://auth0.com/docs).
