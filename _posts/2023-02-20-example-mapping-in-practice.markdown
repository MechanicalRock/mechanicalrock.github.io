...
Apu (the store owner) has decided to go digital to boost business and support his growing family. The opening requirement as an example map might pan out as follows:
...
And all that is based on the fact that the developer even read the spec.
...
Turns out that only two of the five personas can actually Approve the changes, and only if they are in Pending Approval state!

Subsequenely the code gets thrown in the bin and Story Development restarts.
...
Arming the developer and tester with all the info they could possibly want. This often occurs when someone writes a func spec in Given, When, Then syntax and places it into the Jira Card to be Agile.
...
We've all heard the saying "a picture tells a thousand words" right...well a thousand is a lot for a set of acceptance criteria on a User Story but it's still a valid point. Pictures can really help solidify an example on a rule. Consider the feature mentioned above, where users get notified of special offers via the Kwik-E-Mart app. Notifications are obviously time and location sensitive, so the Product owner might mention something like "specials should be notified within a 5km range of the store, and only for 5 minutes". Sounds pretty simple right? But don't people with mobile phones move around during that 5 minute period. A sketch here can really solidify what the intention is.
...
A simple story which is easy to understand. This scenario usually leads to a very simple map which has 2-3 rules each with 2-3 examples and no major outstanding questions. Team can leave the session and start developing the feature immediately.
