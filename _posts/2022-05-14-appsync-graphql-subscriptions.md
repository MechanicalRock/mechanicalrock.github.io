---
layout: post
title: AWS Appsync, graphql subscriptions with React useSubscription
...
tags: ["AWS Appsync", "Graphql", "tutorial", "AWS_Annotations", "subscription", "Real-time data"]
---
...
Short and sweet: A query language for your API.

GraphQL gives client the power to ask for exactly what they need and get back predictable results. Making updates to data is made simple with graphql mutations, allowing developers to describe how the data should change.
...
As we said earlier, there are use-cases when our application client expects real-time data from the server. One way to achieve this is by using WebSockets, which establish a two-way interactive communication session between the user’s browser and a server. Subscriptions are another and more efficient way to send data from the server to the client. Those are very similar to queries, but instead of immediately returning a single answer, a result is sent every time a particular event happens on the server without the client needing to resend that request.
...
As the schema shows, we have a mutation which updates the status of an order in a data source like our database of choice. We want our UI gets the live status of the order, each time an event updates the status.
...
1- The subscription response must be optional. AppSync allows us to successfully define the schema with a required subscription response, but when we hooked the the subscription to our frontend client, it threw some error that subscription responses must be optional.
...
2- A subscription response contains only the predicted fields which were returned by the mutation, all the other fields will be null. For instance, if we have a mutation which return the `orderStatus` only and not the entire order object:
...
3- Subscription arguments are indeed filters that specifies when the server to notify the client.
...
However, if you want to enable client to subscribe to a single order `OR` all orders, you can make this argument optional by removing the exclamation point (!) from the subscription prototype.
...
The golden point here to consider is that, If the filtering field is not specified in the mutation response, the subscription is not fired! If we define the addOrder mutation as bellow ( createdAt filed has intentionally been removed from mutation response):
...
We reviewed that what are AWS AppSync, GraphQL. Followed by steps on how to implement real time session between client and the server with AWS `@aws_Subscribe` directive in AppSync. And finally wrapped it up with troubleshooting, that saves you the hours of frustration and debugging!
